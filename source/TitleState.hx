...
class TitleState extends MusicBeatState
{
	...
	public var forceCheat:Bool = false;
	public static var curSong:String = null;
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.F10)
			MainMenuState.forceChange = true;
		...
		if (pressedEnter && !transitioning && skippedIntro && CachedFrames.cachedInstance.loaded)
		{
			#if !switch
	...
	public static function musicShit():Void //GREAT FUNCTION THANKS
	{
			...
			trace(Paths.fileExists(Paths.voices(song), MUSIC, true) + ' | ' + FileSystem.exists(Paths.voices(song)));

			if (FileSystem.exists(Paths.voices(song)))
				FlxG.sound.playMusic(Paths.voices(song), 0.6, true);
			//curSong = data[0]; //need help with vocals so fuck this

			MainMenuState.curSong = song;

			trace(song);
		//	TitleState.instance.addSongText();
	...
	override function beatHit()
	{
		...
		{
			case 1:
				createCoolText(['discussions', 'ThatPizzaTowerFan', 'ekajahplA']);
			...
			// credTextShit.screenCenter();
			case 5:
					createCoolText(['Immortalism Mod', 'thanks a lot']);
			case 7:
					addMoreText('comments would help');
			// credTextShit.text += '\nNewgrounds';
			case 8:
				deleteCoolText();
				ngSpr.visible = false;
	...
	{
		if (!skippedIntro && FlxG.save.data.noNull_t != null)
		{
			remove(ngSpr);
