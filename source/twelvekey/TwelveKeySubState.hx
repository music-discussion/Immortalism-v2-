...
class TwelveKeySubState extends MusicBeatSubstate
{
    ...
    var fullscreenBind:String = FlxG.save.data.F11Bind;
    var fullscreenText:FlxText = new FlxText();

    var pauseKey:FlxSprite = new FlxSprite();
    var pauseBind:String = FlxG.save.data.pauseBind;
    ...
    override function update(elapsed:Float) {
        ...
        if(arrow_Group != null)
        {
            ...
            if(back)
            {
                ...
                FlxG.save.data.F11Bind = fullscreenBind;
                FlxG.save.data.killBnd = killBind;
                FlxG.save.data.pauseBind = pauseBind;
            ...
            {
                #if debug
                if (FlxG.keys.pressed.SIX && FlxG.keys.pressed.NINE)
                    key_Count = 18;
                #end
                if(leftP)
                    changeKeyCount("down");
    ...
    function changeKeyCount(Direction:String)
    {
        switch (Direction)
        {
            case 'up':
                var keyC = [4, 9, 12];
                for (i in 0...keyC.length)
                {
                    if (key_Count == keyC[i] && !notSpam)
                    {
                        trace(keyC[i] + ' | ' + Direction);
                        switch (keyC[i])
                        {
                            case 4:
                                key_Count = 9;
